...
describe('parseSearchResults', () => {
  it('extrait uniquement la section randonnées', () => {
    ...
    expect(parseSearchResults(html, 10)).toEqual([
      {
        title: 'Lac Bleu',
        url: 'https://www.visorando.com/randonnee-lac-bleu/',
        activity: 'Pédestre',
        distanceKm: 10.54,
      },
    ]);
...
describe('parseHikeDetails', () => {
  it('normalise les métriques françaises et les coordonnées ouest', () => {
    ...
    expect(parseHikeDetails(html, 'https://www.visorando.com/randonnee-test/')).toMatchObject({
      id: '145444',
      distanceKm: 10.47,
      durationMinutes: 185,
      elevationGainM: 1202,
  ...
  it('rejette une page sans fiche', () => {
    expect(() =>
      parseHikeDetails('<html></html>', 'https://www.visorando.com/randonnee-test/'),
    ).toThrow(/fiche/iu);
  });

  it('utilise les coordonnées HTML et ignore un JSON-LD invalide', () => {
    const html = `<h1>Traversée</h1><script type="application/ld+json">{invalide</script><section><h2>Fiche technique</h2><div class="vr-walk-datasheet--dataset"><strong>Durée moyenne :</strong>45 min</div><div class="vr-walk-datasheet--dataset"><strong>Retour au départ :</strong>Non</div><div class="vr-walk-datasheet--dataset"><strong>Départ/Arrivée :</strong>S 21,25° / E 55,50°</div></section>`;
    expect(parseHikeDetails(html, 'https://www.visorando.com/randonnee-test/')).toMatchObject({
      durationMinutes: 45,
      loop: false,
      latitude: -21.25,
