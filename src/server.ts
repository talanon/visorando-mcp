...
const errorResult = (error: unknown) => ({
  content: [{ type: 'text' as const, text: error instanceof Error ? error.message : 'Erreur inconnue.' }],
  isError: true,
});
...
export function createServer(client = new VisorandoClient()): McpServer {
  ...
  server.registerTool('search_hikes', {
    ...
    inputSchema: {
      query: z.string().trim().min(3).max(120).describe('Lieu, titre, mot-clé ou numéro de randonnée'),
      limit: z.number().int().min(1).max(20).default(10),
    },
  }, async ({ query, limit }) => {
    try { return toToolResult(await client.searchHikes(query, limit)); }
    catch (error) { return errorResult(error); }
  });

  server.registerTool('get_hike', {
    title: 'Consulter une randonnée Visorando',
    description: 'Lit les métadonnées publiques d’une fiche sans reproduire sa description ni sa trace protégées.',
    inputSchema: {
      reference: z.string().trim().min(1).max(500).describe('URL Visorando, slug randonnee-… ou numéro'),
    },
  }, async ({ reference }) => {
    try { return toToolResult(await client.getHike(reference)); }
    catch (error) { return errorResult(error); }
  });
  ...
  server.registerTool('compare_hikes', {
    ...
    try { return toToolResult(await Promise.all(references.map((reference) => client.getHike(reference)))); }
    catch (error) { return errorResult(error); }
  });
  ...
  server.registerResource(
    ...
    { title: 'Périmètre légal et technique', description: 'Limites volontaires du connecteur non officiel.' },
    (uri) => Promise.resolve({ contents: [{
      uri: uri.href,
  ...
  server.registerPrompt('choose_a_hike', {
